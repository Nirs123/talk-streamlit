...
with st.sidebar:
    page = st.selectbox("Sélectionnez votre page", options=["Explorer les données", "Modifier les données"])
...
@st.dialog("Confirmer la sauvegarde")
def confirm_save():
    st.write("Êtes vous sur de vouloir sauvegarder le dataset ?")
    if st.button("OK"):
        new_dataset.to_csv("pokemon_dataset.csv")
        st.rerun()

if page == "Modifier les données":
    st.title("Modification du dataset pokemon")
    
    dataset = pd.read_csv("pokemon_dataset.csv")
    
    new_dataset = st.data_editor(dataset)

    if st.button("Sauvegarder le dataset"):
        confirm_save()
